...
import { ChainCacheService } from '../services/chain-cache.service'
...
export class EventController {
  ...
  async findEventById(
    ...
    // Hold-until-first-use: filter held Kind 297 delegate entries from untrusted callers
    if (event.kind === 297 && this.chainCacheService.isHeld(event.id)) {
      const isTrusted = pubkey && this.chainCacheService.isTrustedService(pubkey)
      if (!isTrusted) {
        throw new NotFoundException('Event not found')
      }
    }
  ...
  async findEvents(
    ...
    const isTrusted = pubkey && this.chainCacheService.isTrustedService(pubkey)
    const filteredEvents = isTrusted
      ? events
      : events.filter((e) => !(e.kind === 297 && this.chainCacheService.isHeld(e.id)))
