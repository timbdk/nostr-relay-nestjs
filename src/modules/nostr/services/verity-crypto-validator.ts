...
/**
 * Synchronous Verity event verification using @noble/curves and Node.js crypto.
 *
...
export function verifyVerityEventSync(
  ...
  if (hasKid && (typeof event.kid !== 'string' || !/^[0-9a-f]{64}$/i.test(event.kid))) {
    return 'invalid: kid format is wrong'
  }

  if (hasKey && (typeof event.key !== 'string' || !event.key.includes(':'))) {
    return 'invalid: key format is wrong'
  }

  // 2. Ambiguity check: both kid and key present
  if (hasKid && hasKey) {
    return 'invalid: ambiguous — both kid and key present'
  }

  // 3. ID recompute: 8-slot canonical serialization
  // TODO (Phase 02 / Set B): Converge on serializeEvent from the fork once it accepts serializationPrefix directly.
  try {
